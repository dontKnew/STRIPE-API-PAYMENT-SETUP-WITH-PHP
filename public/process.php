    ...
    if($stripe->addCustomerToStripe($data)){
        ...
        if($stripe->addPaymentToStripe($data)){
            ...
            if($stripe->saveData("transaction",$data)){
                $_SESSION['msg'] = "<div class='alert alert-success' role='alert'>". $stripe->message[2]['msg']."</div>";   
            }else {
                $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'>". $stripe->message[2]['msg']."</div>";   
            }
        }else {
            $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'>". $stripe->message[1]['msg']."</div>";   

        }
    }else {
        $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'>". $stripe->message[0]['msg']."</div>";   
    }
}else {
    $_SESSION['msg'] = "<div class='alert alert-danger' role='alert'> Something wrong, Please try again later </div>";   
}
header("location:./");
